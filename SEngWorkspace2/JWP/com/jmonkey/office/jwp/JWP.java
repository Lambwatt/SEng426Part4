...
import jmonkey.office.jwp.support.PropertySheetDialog;
...
public class JWP extends JFrame implements ActionListener {
	...
	private static final String rbn = "jmonkey.office.jwp.JWPResources";
	...
	protected final ActionListener m_openWindowAction = new ActionListener() {
		public void actionPerformed(ActionEvent e) {
			...
			DocumentFrame frame = getDesktopManager().getOpenDocument(
					e.getActionCommand());
	...
	protected final class MainDesktop extends JDesktopPane implements
			Scrollable, AdjustmentListener {
	...
	protected class ColourActionCellRenderer extends JLabel implements
		...
		public Component getListCellRendererComponent(JList list, Object value,
			...
			if (value != null) {
				...
				String prop = getRegistry()
						.getString("COLOURS", (String) value);
	...
	protected final class PrintAction extends AbstractAction {
		public PrintAction() {
			super("Print...");
			boolean enabled = getRegistry().getBoolean("MAIN",
	...
	public void doExit() {
		...
		reg.setProperty("USER", "show.file.toolbar", getFileToolBar()
				.isVisible());
		reg.setProperty("USER", "show.format.toolbar", getFormatToolBar()
				.isVisible());
		...
		for (int p = 0; p < m_fileHistory.getItemCount(); p++) {
			reg.setProperty("FILE_HISTORY", m_fileHistory.getItem(p).getText(),
					...
					reg.initGroup(
							...
							new String[][] {
									...
									{ "mdi.outline.drag", "true", "boolean" },
									{
											...
											"String" },
									{ "new.document.title", "New Document",
											"String" },
	...
	private void init() {
		...
		fileToolPanel.add(getFileToolBar(),
				reg.getString("USER", "position.file.toolbar"));

		JPanel formatToolPanel = new JPanel();
		formatToolPanel.setLayout(new BorderLayout());
		formatToolPanel.add(getFormatToolBar(),
				reg.getString("USER", "position.format.toolbar"));
	...
	public static void main(String[] args) {
		try {
			...
			System.err
					.println("Offending registry file is " + ex.getFileName());
			System.exit(1);
		}
	}
